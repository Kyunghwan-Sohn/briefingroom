...
<nav class="bnav"><a class="on" href="/"><span style="font-size:16px;font-weight:700">B</span>브리핑</a><a href="#"><span style="font-size:16px">⌕</span>검색</a><a href="#"><span style="font-size:16px">≡</span>달력</a><a href="/finlaw/"><span style="font-size:16px;font-weight:700">L</span>법령AI</a><a href="https://t.me/govbrief"><span style="font-size:16px">→</span>알림</a></nav>
  ...
  try {{
    ...
    if (data.results && data.results.length) {{
      searchResults.innerHTML = data.results.map(r =>
        '<a href="' + r.link + '" style="display:block;padding:12px 16px;border-bottom:1px solid #e0e0e0;text-decoration:none;color:#222">' +
        '<div style="display:flex;align-items:center;gap:8px"><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:4px;background:rgba(217,108,44,.06);color:#d96c2c">' + (typeLabel[r.type]||'') + '</span>' +
        '<span style="font-size:14px;font-weight:600">' + r.title.slice(0,50) + '</span></div>' +
        '<div style="font-size:12px;color:#999;margin-top:3px">' + r.subtitle.slice(0,60) + '</div></a>'
      ).join('') + '<a href="/tools/finlaw-gpt/?q=' + encodeURIComponent(query) + '" style="display:block;padding:12px 16px;text-align:center;font-size:13px;color:#d96c2c;font-weight:600;text-decoration:none">FinLaw GPT에서 상세 질문 →</a>';
      searchResults.style.display = 'block';
    }} else {{
      searchResults.innerHTML = '<div style="padding:16px;text-align:center;font-size:13px;color:#999">검색 결과 없음</div><a href="/tools/finlaw-gpt/?q=' + encodeURIComponent(query) + '" style="display:block;padding:12px 16px;text-align:center;font-size:13px;color:#d96c2c;font-weight:600;text-decoration:none">FinLaw GPT에게 질문하기 →</a>';
      searchResults.style.display = 'block';
    }}
